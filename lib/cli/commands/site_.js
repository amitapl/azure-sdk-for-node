...
var scriptGenerator = require('./scriptgenerator/generator');
...
exports.init = function (cli) {
  ...
  site.command('deploymentscript')
        ...
        .option("--wap <projectFilePath>", "Create a deployment script for .NET web application, specify the project file path")
        .option("--website", "Create a deployment script for basic website")
        .option("--node", "Create a deployment script for node.js website")
        .option("-s, --solutionFile [file path]", "The solution file path (sln)")
        .option("-t, --scriptType [batch|sh]", "The script output type (default: batch)")
        .execute(function (name, options, _) {
            var repositoryRoot = options.repositoryRoot || '.';
            var scriptType = options.scriptType || 'batch';
            var projectFile = options.wap;
            var solutionFile = options.solutionFile;

            var specifyOneFlagError = 'Please specify only one of these flags: --wap, --website or --node';

            var projectType;
            if (options.wap) {
                if (options.website || options.node) {
                    throw new Error(specifyOneFlagError);
                }
                ...
                throw new Error('Missing project type (--wap, --website or --node');
            }

            try {
                scriptGenerator.generateDeploymentScript(repositoryRoot, projectType, projectFile, solutionFile, scriptType, log);
            }
            catch (e) {
                log.error(e.message);
            }
