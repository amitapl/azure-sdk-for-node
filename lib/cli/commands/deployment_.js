...
exports.init = function (cli) {
  ...
  scm.command('generate')
    .whiteListPowershell()
    .usage('[options]')
    .description('Generate custom deployment script')
    .option("-r, --repositoryRoot [dir path]", "Root path for the repository (default: .)")
    .option("-t, --projectType <projectType>", "The project type (wap|website|nodejs|other)")
    .option("-p, --projectFile [file path]", "Project file path (only for wap)")
    .option("-s, --solutionFile [file path]", "Solution file path (only for wap or website)")
    .execute(function (name, options, _) {
        ...
        try {
            scriptGenerator.generateDeploymentScript(repositoryRoot, projectType, projectFile, solutionFile, log);
        }
        catch (e) {
            log.error(e.message);
        }
